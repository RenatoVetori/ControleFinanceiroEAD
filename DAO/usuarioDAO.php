...
class UsuarioDAO extends Conexao
{
    ...
    public function CarregarMeusDados()
    {
        ...
        //remove o index dentro do array permanecendo somente com as colunas do BD
    ...
    public function ValidarCadastro($nome, $email, $senha, $confsenha)
    {
        if (trim($nome) == '' || trim($email) == '' || trim($senha) == '' || trim($confsenha) == '') {
            return 0;
        }
        if (strlen($senha) < 6) {
            return -2;
        }
        if (trim($senha) != trim($confsenha)) {
            return -3;
        }
